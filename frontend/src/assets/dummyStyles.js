...
export const styles = {
  ...
  `p-3 pt-4 transition-all duration-300 ${sidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64'}`,
  },
...
subtitle: "text-sm text-gray-500",
  ...
  // Stat Card Styles
statCards: {
  grid: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-4",
  card: "bg-white p-3 rounded-xl shadow-sm border border-gray-100",
  cardHeader: "flex justify-between items-start",
  cardTitle: "text-xs text-gray-600",
  cardValue: "text-lg font-semibold text-gray-800 mt-1",
    iconContainer: (color) => `bg-${color}-100 p-1.5 rounded-md`,
    icon: (color) => `w-4 h-4 text-${color}-600`,
...
rightColumn: "lg:col-span-1 space-y-4",
  },
  ...
  transactions: {
    ...
    listContainer: "space-y-2 max-h-[400px] overflow-y-auto",
    transactionItem: "flex justify-between items-center p-2 hover:bg-gray-50 rounded-lg border border-gray-100", 
    iconWrapper: (type) => type === 'income' ? 'bg-teal-100 text-teal-600' : 'bg-orange-100 text-orange-600',
    icon: "w-4 h-4",
    details: "min-w-0",
    description: "text-sm font-medium text-gray-800 truncate",
meta: "text-[11px] text-gray-500",
  ...
  categories: {
    ...
    list: "space-y-4",
   categoryItem: "flex items-center justify-between text-sm",
   categoryIconContainer: "bg-gray-100 p-1.5 rounded-md",
    ...
    summaryValue: "text-base font-semibold text-gray-800",
  },
