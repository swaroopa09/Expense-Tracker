...
import { ArrowDown, BarChart2, ChevronDown, ChevronUp, DollarSign, PiggyBank, Plus, ShoppingCart, TrendingDown, TrendingUp, Wallet, TrendingUp as ProfitIcon, PieChart as PieChartIcon, PieChart } from 'lucide-react';
...
const Dashboard = () => {
 ...
 const financialOverviewData = useMemo(() => {
  ...
  // 🔥 ALWAYS use spendByCategory first (your data is coming here)
  if (overviewMeta.spendByCategory && Object.keys(overviewMeta.spendByCategory).length > 0) {
    return Object.keys(overviewMeta.spendByCategory).map((key) => ({
      name: key,
      value: Number(overviewMeta.spendByCategory[key]) || 0,
    }));
  }
  ...
  item.type || (item.source ? "income" : "expense");
        ...
        setOverviewMeta((prev) => ({
         ...
         expenseDistribution:
  Array.isArray(data.expenseDistribution) &&
  data.expenseDistribution.length > 0
    ? data.expenseDistribution
    : Object.keys(data.spendByCategory || {}).map((key) => ({
        category: key,
        amount: data.spendByCategory[key],
      })),
        ...
        <div className={dashboardStyles.pieChartHeight}>
          <ResponsiveContainer width="100%" height="100%">
            <PieChart className={chartStyles.pieChart}>
              <Pie
  data={financialOverviewData.length ? financialOverviewData : [{ name: "No Data", value: 1 }]}
                cx="50%"
                cy="50%"
                innerRadius={70}
                outerRadius={110}
                ...
                labelLine={false}
              >
                {(financialOverviewData.length ? financialOverviewData : [{ name: "No Data", value: 1 }]).map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
